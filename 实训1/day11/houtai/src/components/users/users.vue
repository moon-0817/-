...
<template>
  <div class="users">
    ...
    <div class="box">
      ...
      <!-- 分配权限的弹出框 -->
      <el-dialog title="提示" :visible.sync="garntFlag" width="30%">
        <!-- <span>这是一段信息</span> -->
        <p>当前的用户：{{ garnusername }}</p>
        <p>当前的角色：{{ garnname }}</p>
        <!-- 下拉框 -->
        <el-select v-model="opName" placeholder="请选择">
          <el-option
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="garntFlag = false">取 消</el-button>
          <el-button type="primary" @click="garntAdd">确 定</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async garnt(val) {
      console.log(val);
      this.garntFlag = true;
      this.garnusername = val.username;
      this.garnname = val.role_name;
      this.id = val.id;
    ...
    // 确认分配权限
    async garntAdd() {
      let res = await garnname(this.id, { rid: this.opName });
      console.log(res);
      this.garntFlag = false;
      this.getUsers();
    },
